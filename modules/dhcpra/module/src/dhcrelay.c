...
dhc_add_relay_agent_options(struct dhcp_packet *packet,
    ...
    if (memcmp(packet->options, DHCP_OPTIONS_COOKIE, 4)) {
        debug_counter_inc(&dhc_relay_stat.request_missing_cookie);
        return length;
    }
 ...
 out:
    ...
    if (!is_dhcp) {
        debug_counter_inc(&dhc_relay_stat.request_missing_message);
        return length;
    }
...
check_vlan_by_agent_option(u_int8_t *buf, int len, const uint32_t vlan) {
    ...
    while (i < len) {
        ...
        if (i + 1 == len ||
            i + buf[i + 1] + 2 > len) {
            debug_counter_inc(&dhc_relay_stat.reply_corrupt_option);
            return -1;
        }
        switch(buf[i]) {
    ...
    /* If there's no circuit ID, it's not really ours, tell the caller
       it's no good. */
    if (!circuit_id) {
        debug_counter_inc(&dhc_relay_stat.reply_missing_circuit_id);
        return -1;
    }
...
dhc_strip_relay_agent_options(struct dhcp_packet *packet,
    ...
    /* If there's no cookie, it's a bootp packet, so we should just
       forward it unchanged. */
    if (memcmp(packet->options, DHCP_OPTIONS_COOKIE, 4)) {
        debug_counter_inc(&dhc_relay_stat.reply_missing_cookie);
        return length;
    }
 ...
 out:
    /* If it's not a DHCP packet, we're not supposed to touch it. */
    if (!is_dhcp) {
        debug_counter_inc(&dhc_relay_stat.reply_missing_message);
        return length;
    }
    
    if (!dhcp_agent_option) {
        debug_counter_inc(&dhc_relay_stat.reply_missing_option);
    }
