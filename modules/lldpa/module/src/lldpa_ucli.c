...
#define LLDPA_HEXDUMP_TRACE(fmt, ...)                       \
            ucli_printf(ucg, fmt, ##__VA_ARGS__)
#include "lldpa_int.h"
...
lldpa_show_portcounters__(ucli_context_t* uc, uint32_t port_no)
{
    ...
    ucli_printf(uc, "%d\t%d\t%d\t%"PRId64"\t%"PRId64"\t%"PRId64"\t%"PRId64"\t%"PRId64"\t%"PRId64"\n" , 
...
lldpa_ucli_ucli__show_lldpa_portcounters__(ucli_context_t* uc)
{
    ...
    ucli_printf(uc, "PORT	of port number\n"
                "r_intv	rx time interval\n"
                "t_intv	tx time interval\n"
                "pkt_in	num of packets in recv\n"
                "pk_out	num of packets out tx to data plane\n"
                "TOmsg	numb of timeout msg to the control plane\n"
                "MM_ND	Mismatched due to no data\n"
                "MM_DD	Data Mismatched\n"
                "MATCHD  Data matched\n");

    ucli_printf(uc, "PORT\tr_intv\tt_intv\tpkt_in\tpk_out\tTOmsg\tMM_ND\tMM_DD\tMATCHD\n");
    if (uc->pargs->count == 1) {
